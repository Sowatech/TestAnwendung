...
export class DragulaService {
    ...
    //-- friendlier swt events:
    public onDrop: EventEmitter<IDropEvent> = new EventEmitter();
    public onRemove: EventEmitter<IRemoveEvent> = new EventEmitter();
 ...
 public add(name: string, drake: any): any {
    ...
    let bagExists = bag ? true : false;
    if (bagExists) {
      console.warn('Bag named: "' + name + '" already exists.');
    ...
    public setOptions(name: string, options: IDragulaBagOptions): void {
        let bag = this.add(name, dragula(<any>options));
            ...
            function replicate(): void {
                ...
                if (type == "drop") {
                    let dropEvent: IDropEvent = {
                        bagName: bag.name,
                        droppedElement: args[0],
                        dropContainer: args[1],
                        sourceContainer: args[2],
                        nextElement: args[3]
                    }
                    that.onDrop.emit(dropEvent);
                }

                if (type == "remove") {
                    let removeEvent: IRemoveEvent = {
                        bagName: bag.name,
                        droppedElement: args[0],
                        dropContainer: args[1],
                        sourceContainer: args[2],
                    }
                    that.onRemove.emit(removeEvent);
                }
            }
...
export interface IDragulaBagOptions {
    //TO Continue
    removeOnSpill?: boolean,
    moves?: (item: HTMLElement, container: HTMLElement, handle: HTMLElement, nextElement: HTMLElement) => boolean,
    accepts?: (item: HTMLElement, target: HTMLElement, container: HTMLElement, nextElement: HTMLElement) => boolean
}